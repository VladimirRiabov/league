...
class NetworkCore {
    ...
    private let token = "pk_1dcd79f57ecd4998ba8a6efaed220b99"
...
extension NetworkCore: NetworkCoreProtocol {
    func request<Res: Responsable>(metadata: String, completion: @escaping (CoreResult<Res>) -> Void) {
        guard let urlRequest = URL(string: "\(url)/\(metadata)?token=\(token)") else {completion(.failure(NetworkError.invalidURL))
            return}
